...
func NewTaskManager(cacheDir string) (*TaskManagerImpl, error) {
	...
	tm.db.Exec(`CREATE TABLE IF NOT EXISTS task_plans (id TEXT PRIMARY KEY, title TEXT, description TEXT, status TEXT, created_at INTEGER)`)
	tm.db.Exec(`CREATE TABLE IF NOT EXISTS tasks (id TEXT PRIMARY KEY, plan_id TEXT, parent_id TEXT, title TEXT, description TEXT, status TEXT, task_order INTEGER, dependencies TEXT, files TEXT, result TEXT, error TEXT, created_at INTEGER, started_at INTEGER, completed_at INTEGER, metadata TEXT)`)
	return tm, nil
}
...
func (tm *TaskManagerImpl) CreatePlan(title, desc string) (*domain.DecompTaskPlan, error) {
	...
	tm.db.Exec(`INSERT INTO task_plans VALUES (?,?,?,?,?)`, p.ID, p.Title, p.Description, p.Status, p.CreatedAt.Unix())
	return p, nil
}
...
func (tm *TaskManagerImpl) GetPlan(id string) (*domain.DecompTaskPlan, error) {
	...
	rows, _ := tm.db.Query(`SELECT id,parent_id,title,description,status,task_order,dependencies,files,result,error,created_at,started_at,completed_at,metadata FROM tasks WHERE plan_id=? ORDER BY task_order`, id)
	if rows != nil {
		defer rows.Close()
		for rows.Next() {
			...
			rows.Scan(&t.ID, &pid, &t.Title, &t.Description, &t.Status, &t.Order, &deps, &files, &res, &er, &tca, &sa, &coa, &meta)
			if pid.Valid {

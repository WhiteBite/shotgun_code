...
function _updateParentSelection(nodesMap: Map<string, FileNode>, path: string | null) {
  if (path === null) return;
  const parent = nodesMap.get(path);
  if (!parent || !parent.children) return;

  const children = parent.children.map(c => nodesMap.get(c.path)!).filter(Boolean);
...
export function toggleNodeSelection(nodesMap: Map<string, FileNode>, path: string) {

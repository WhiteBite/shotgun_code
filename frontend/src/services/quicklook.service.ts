...
export function computePanelPos(event?: MouseEvent) {
  const safeTop = (y: number) => Math.max(8, Math.min(y, window.innerHeight - 160));
  const safeLeft = (x: number) => Math.max(8, Math.min(x, window.innerWidth - 620));
  if (!event) {
    return { top: safeTop(120) + "px", left: safeLeft(window.innerWidth / 2 - 300) + "px" };
  }
  return { top: safeTop(event.clientY) + "px", left: safeLeft(event.clientX) + "px" };
}
...
export async function loadAndHighlight(
  ...
  try {
    ...
    if (type === "fs") {
      ...
      if (!commitHash) throw new Error("commitHash is required for git preview");

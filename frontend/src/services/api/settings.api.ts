...
export const settingsApi = {
    getSettings: (): Promise<domain.SettingsDTO> =>
        apiCall(() => wails.GetSettings(), 'Failed to load settings.', 'settings'),

    saveSettings: (settings: string): Promise<void> =>
        apiCall(() => wails.SaveSettings(settings), 'Failed to save settings.', 'settings'),
    ...
    getCustomIgnoreRules: (): Promise<string> =>
        apiCall(() => wails.GetCustomIgnoreRules(), 'Failed to load custom ignore rules.', 'settings'),

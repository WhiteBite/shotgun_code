    ...
    >
      <div class="bg-gray-800 rounded-lg shadow-2xl w-full max-w-4xl max-h-[85vh] flex flex-col border border-gray-700">
        <header class="p-4 border-b border-gray-700 flex-shrink-0 flex justify-between items-center">
          <h2 class="text-xl font-semibold text-white">{{ t('commits.title') }}</h2>
          <button @click="gitStore.hideHistory" class="p-2 rounded-md hover:bg-gray-700" aria-label="Close">
        ...
        <main class="p-4 overflow-y-auto flex-grow">
          <div v-if="gitStore.isLoading" class="text-center text-gray-400">{{ t('commits.loading') }}</div>
          <div v-else-if="gitStore.commits.length === 0" class="text-center text-gray-400">{{ t('commits.empty') }}</div>
          <div v-else class="space-y-2">
            <div v-for="commit in gitStore.commits" :key="commit.hash" class="p-3 bg-gray-900/50 rounded-md border border-gray-700">
              <div class="flex gap-3">
                ...
                <div class="flex-grow min-w-0">
                  <p class="font-semibold text-white font-mono text-sm truncate">{{ commit.subject }}</p>
        ...
        <footer class="p-4 border-t border-gray-700 flex justify-between items-center flex-shrink-0">
          ...
          <div class="flex gap-3">
            <button @click="gitStore.hideHistory" class="px-4 py-2 bg-gray-600 hover:bg-gray-500 rounded-md">{{ t('button.cancel') }}</button>
            <button @click="applySelection" :disabled="selectedCommits.length === 0" class="px-4 py-2 bg-blue-600 hover:bg-blue-500 rounded-md disabled:opacity-50">{{ t('button.addFiles') }}</button>
          </div>
        </footer>
      </div>
    </div>
...
function onKeydown(e: KeyboardEvent) { if (e.key === "Escape") gitStore.hideHistory(); }

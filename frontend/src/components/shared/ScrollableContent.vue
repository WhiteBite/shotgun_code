    ...
    >
      <template v-if="!highlight && virtualize && lines.length > 100">
        ...
        >
          <template #default="{ item }">
            <div class="flex">
              <div class="w-12 text-right pr-3 text-gray-500 text-xs select-none">
                {{ item.i + 1 }}
              </div>
              <pre class="flex-1 text-sm font-mono text-gray-300 whitespace-pre-wrap select-text m-0 leading-relaxed">
                <code>{{ item.t }}</code>
              </pre>
            </div>
          </template>
      ...
      <template v-else>
        <div v-if="!highlight && content" class="code-content">
          ...
          >
            <div class="w-12 text-right pr-3 text-gray-500 text-xs select-none border-r border-gray-700">
              {{ index + 1 }}
            </div>
            <pre class="flex-1 text-sm font-mono text-gray-300 whitespace-pre-wrap select-text m-0 leading-relaxed pl-3">
              <code>{{ line.t }}</code>
            </pre>
          </div>
        </div>
...
const showScrollBottom = computed(() => scrollTop.value + clientHeight.value < scrollHeight.value - 200)
...
function handleScroll() {
  ...
  const position = scrollHeight.value > clientHeight.value 
...
watch(() => props.currentSearchIndex, (newIndex) => {

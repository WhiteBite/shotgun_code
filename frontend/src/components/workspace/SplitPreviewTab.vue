  ...
  <div class="h-full flex flex-col">
    ...
    <div class="p-3 border-b border-gray-700 bg-gray-800/30 flex-shrink-0">
      <div class="text-xs text-gray-400 mb-2">
        ...
        ({{ splitPreview?.totalTokens?.toLocaleString() || 0 }} tokens total)
      </div>
    ...
    <div class="flex-1 min-h-0 relative">
      <div v-if="!selectedChunkContent" class="flex items-center justify-center h-full text-gray-500">
    ...
    <!-- Chunk Stats Footer -->
    <div v-if="selectedChunkData" class="p-2 border-t border-gray-700 bg-gray-800/30 flex-shrink-0">
      <div class="flex items-center justify-between text-xs text-gray-400">
        <div>
          Part {{ selectedChunk + 1 }} of {{ splitPreview?.chunks?.length || 0 }}: 
          {{ selectedChunkData.chars.toLocaleString() }} chars,
          ~{{ selectedChunkData.tokens.toLocaleString() }} tokens
        </div>
...
const selectedChunkData = computed(() => {
  if (!props.splitPreview?.chunks || selectedChunk.value >= props.splitPreview.chunks.length) {
    return null;
  }
...
function copyCurrentChunk() {
  ...
  notifications.addLog(`Part ${selectedChunk.value + 1} copied to clipboard`, 'success');
...
function copyAllChunks() {
  ...
  const allText = props.splitPreview.chunks.map((chunk, index) => 
    `=== Part ${index + 1} ===\n${chunk.text}\n`
  ).join('\n');
...
function nextChunk() {
  if (props.splitPreview?.chunks && selectedChunk.value < props.splitPreview.chunks.length - 1) {
    selectedChunk.value++;
  }
}
</script>

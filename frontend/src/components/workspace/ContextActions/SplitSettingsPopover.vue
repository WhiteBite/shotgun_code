<template>
  <div class="relative">
    ...
    >
      ...
      <div class="text-[11px] text-gray-400 mt-2 min-h-[2rem] p-2 bg-gray-900/50 rounded border border-gray-600">
        <template v-if="preview && local.enableAutoSplit">
          ...
          <div v-if="preview.chunks.length > 0" class="mt-1">
            Avg per chunk: ~{{ Math.round(preview.totalTokens / preview.chunkCount).toLocaleString() }} tokens
          </div>
          <div v-if="preview.warnings && preview.warnings.length > 0" class="mt-1 text-yellow-400">
      ...
      <div class="flex justify-end gap-2 mt-2">
        ...
        <button type="button" class="px-2 py-1 text-xs bg-blue-600 hover:bg-blue-500 rounded text-white" @click="apply">
          Apply
        </button>
      </div>
    </div>

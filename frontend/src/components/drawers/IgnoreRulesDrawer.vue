      ...
      <div class="flex-grow flex flex-col min-h-0">
        <div class="flex-shrink-0 border-b border-gray-700 mb-2">
          <nav class="-mb-px flex space-x-4" aria-label="Tabs">
            <button @click="activeTab = 'custom'" :class="['whitespace-nowrap py-2 px-1 border-b-2 font-medium text-sm', activeTab === 'custom' ? 'border-blue-500 text-blue-400' : 'border-transparent text-gray-400 hover:text-gray-200 hover:border-gray-500']">
              Custom Rules
            </button>
            <button @click="activeTab = 'gitignore'" :class="['whitespace-nowrap py-2 px-1 border-b-2 font-medium text-sm', activeTab === 'gitignore' ? 'border-blue-500 text-blue-400' : 'border-transparent text-gray-400 hover:text-gray-200 hover:border-gray-500']">
              .gitignore (Read-only)
            </button>
          </nav>
        </div>

        <div v-show="activeTab === 'custom'" class="flex-grow flex flex-col min-h-0">
          <p class="text-sm text-gray-400 mb-2 flex-shrink-0">
            Use `.gitignore` syntax. These rules apply when "Custom Rules" is enabled.
          </p>
          <textarea
        ...
        <div v-show="activeTab === 'gitignore'" class="flex-grow flex flex-col min-h-0">
          <p class="text-sm text-gray-400 mb-2 flex-shrink-0">
...
watch(() => uiStore.activeDrawer, async (drawer) => {
  if (drawer === 'ignore') {
    gitignoreContent.value = 'Loading...';
    if(projectStore.currentProject) {
      gitignoreContent.value = await GetGitignoreContent(projectStore.currentProject.path) || 'No .gitignore file found in project root.';
    } else {

<template>
  <transition name="slide-fade">
    <aside v-if="uiStore.activeDrawer === 'settings'" class="absolute top-0 right-0 h-full w-[28rem] bg-gray-800 border-l border-gray-700 shadow-2xl z-30 p-4 flex flex-col">
      ...
      <div v-else class="flex-grow overflow-y-auto space-y-4 text-sm pr-2 -mr-2">
        ...
        <div v-if="settingsStore.settings.selectedProvider === 'openai'" class="p-3 bg-gray-900/50 rounded-md space-y-3 border border-gray-700">
          ...
          <input type="password" v-model="settingsStore.settings.openAIAPIKey" placeholder="OpenAI API Key (sk-...)" class="w-full bg-gray-700 border-gray-600 rounded-md px-3 py-2">
        </div>
        ...
        <div v-if="settingsStore.settings.selectedProvider === 'gemini'" class="p-3 bg-gray-900/50 rounded-md space-y-3 border border-gray-700">
          ...
          <button @click="settingsStore.refreshModels('gemini')" :disabled="settingsStore.isRefreshingModels" class="w-full text-xs py-1 bg-gray-600 hover:bg-gray-500 rounded-md">Обновить модели</button>
        </div>
        ...
        <div v-if="settingsStore.settings.selectedProvider === 'openrouter'" class="p-3 bg-gray-900/50 rounded-md space-y-3 border border-gray-700">
          ...
          <input type="password" v-model="settingsStore.settings.openRouterAPIKey" placeholder="OpenRouter API Key (sk-or-..)" class="w-full bg-gray-700 border-gray-600 rounded-md px-3 py-2">
          <p class="text-xs text-gray-400">Модели жестко заданы. Введите имя модели в поле ниже, если ее нет в списке.</p>
        </div>
        ...
        <div v-if="settingsStore.settings.selectedProvider === 'localai'" class="p-3 bg-gray-900/50 rounded-md space-y-3 border border-gray-700">
          ...
          <input type="text" v-model="settingsStore.settings.localAIHost" placeholder="Host URL (e.g., http://localhost:1234/v1)" class="w-full bg-gray-700 border-gray-600 rounded-md px-3 py-2">
          <input type="text" v-model="settingsStore.settings.localAIModelName" placeholder="Model Name (e.g., llama3)" class="w-full bg-gray-700 border-gray-600 rounded-md px-3 py-2">
          <input type="password" v-model="settingsStore.settings.localAIAPIKey" placeholder="API Key (optional)" class="w-full bg-gray-700 border-gray-600 rounded-md px-3 py-2">
        </div>

        <!-- Model Selection -->
        <div v-if="currentModels.length > 0">
          <label for="model-select" class="block font-medium text-gray-300 mb-1">Модель</label>
          <select id="model-select" v-model="settingsStore.settings.selectedModels[settingsStore.settings.selectedProvider]" class="w-full bg-gray-700 border-gray-600 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">

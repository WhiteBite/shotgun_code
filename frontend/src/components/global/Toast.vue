<template>
  ...
  >
    <div v-if="visible" class="max-w-sm w-full bg-gray-800 shadow-lg rounded-lg pointer-events-auto ring-1 ring-black ring-opacity-5 overflow-hidden border border-gray-600">
      <div class="p-4">
...
onMounted(() => {
  ...
  if (props.duration) {
    setTimeout(() => {
      visible.value = false;
      setTimeout(() => emit('close'), 100); // Wait for transition
    }, props.duration);
  }
});
</script>
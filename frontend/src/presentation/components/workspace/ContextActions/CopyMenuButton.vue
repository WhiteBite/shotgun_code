  ...
  <div class="relative">
    ...
    >
      ...
      <div class="flex flex-col gap-1">
        ...
        <template
          v-if="splitEnabled && splitPreview && splitPreview.chunks.length > 1"
        >
          ...
          >
            Part {{ c.index + 1 }} / {{ splitPreview.chunks.length }} • ~{{
              c.tokens
            }}t
          </button>
        </template>
...
function updateFormat(f: ClipboardFormat) {
  emit("update:exportFormat", f);
}
...
function doCopyAll() {
  emit("do-copy", {
    ...
    stripComments: props.stripComments,
  });
  open.value = false;
}

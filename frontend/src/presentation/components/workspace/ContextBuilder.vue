  ...
  <div class="h-full flex flex-col bg-gray-800">
      ...
      <div class="flex items-center justify-between mb-2">
        ...
        <div class="flex items-center space-x-2">
          <div class="flex items-center space-x-1 text-sm text-gray-400">
            <span>{{ contextStore.selectedFiles.length }} files</span>
            <span>•</span>
    ...
    >
      ...
      <div class="flex-1 overflow-auto">
        <div 
          v-if="contextStore.selectedFiles.length === 0"
...
const selectedFilesList = computed({
  get: () => [...contextStore.selectedFiles],
  set: (value) => {
    contextStore.setSelectedFiles(value)
  }
})

const estimatedTokens = computed(() => {
  return contextStore.contextMetrics.tokenCount || 
         contextStore.selectedFiles.length * 500 // Rough estimate
})
...
export default {
  name: '${getFileName(filePath).replace(/\\..*$/, '')}',
  setup() {
    const count = ref(0)

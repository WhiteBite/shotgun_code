...
const sanitizeMaxTokensPerChunk = () => {
  ...
  if (Number.isNaN(val)) {
    localSettings.value.maxTokensPerChunk = 8000;
  } else {
    localSettings.value.maxTokensPerChunk = Math.min(50000, Math.max(1000, val));
  }
  updateSettings();
};
...
const sanitizeOverlapSize = () => {
  ...
  if (Number.isNaN(val)) {
    localSettings.value.overlapSize = 100;
  } else {
    localSettings.value.overlapSize = Math.min(2000, Math.max(0, val));
  }
  updateSettings();
};
</script>
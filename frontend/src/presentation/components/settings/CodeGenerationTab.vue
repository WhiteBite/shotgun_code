...
const creativityClass = computed(() => {
  const temp = localSettings.value.temperature;
  if (temp >= 1.5) return 'bg-purple-400';
  if (temp >= 1.0) return 'bg-blue-400';
  if (temp >= 0.5) return 'bg-green-400';
  return 'bg-gray-400';
});

const creativityTextClass = computed(() => {
  const temp = localSettings.value.temperature;
  if (temp >= 1.5) return 'text-purple-400';
  if (temp >= 1.0) return 'text-blue-400';
...
const creativityText = computed(() => {
  const temp = localSettings.value.temperature;
  if (temp >= 1.5) return 'Very High';
  if (temp >= 1.0) return 'High';
...
const reliabilityClass = computed(() => {
  const temp = localSettings.value.temperature;
  if (temp <= 0.3) return 'bg-green-400';
  if (temp <= 0.7) return 'bg-blue-400';
  if (temp <= 1.2) return 'bg-yellow-400';
  return 'bg-red-400';
});

const reliabilityTextClass = computed(() => {
  const temp = localSettings.value.temperature;
  if (temp <= 0.3) return 'text-green-400';
  if (temp <= 0.7) return 'text-blue-400';
...
const reliabilityText = computed(() => {
  const temp = localSettings.value.temperature;
  if (temp <= 0.3) return 'Very High';
  if (temp <= 0.7) return 'High';
...
const sanitizeMaxTokens = () => {
  ...
  if (Number.isNaN(val)) {
    localSettings.value.maxTokens = 2000;
    return;
  }
  localSettings.value.maxTokens = Math.min(8000, Math.max(100, val));
  onSettingsChange();
};
</script>

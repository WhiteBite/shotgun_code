...
import type { ContextChunk, PaginationOptions } from '@/domain/entities/ContextSummary';
...
export function usePaginatedContext(contextId?: string, linesPerPage: number = 100) {
  ...
  const loadChunk = async (startLine: number, lineCount: number = linesPerPage): Promise<ContextChunk | null> => {
    const activeContextId = contextId || contextStore.getCurrentContextId();
    ...
    try {
      const chunk = await contextStore.getContextContent(activeContextId, startLine, lineCount);

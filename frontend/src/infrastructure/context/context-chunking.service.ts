...
export class ContextChunkingService {
  ...
  chunkContent(
    ...
    switch (config.strategy) {
      case 'natural':
        return this.chunkByNaturalBreaks(content, lines, config, contentStructure)
      case 'aggressive':
        return this.chunkByTokenLimit(content, lines, config)
      default:
        return this.chunkBalanced(content, lines, config, contentStructure)
    }
  }
  ...
  private chunkBalanced(
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      const shouldBreak = this.shouldBreakAtLine(
        i, 
        lines, 
   ...
   * Check for natural break points
   */
  private isNaturalBreakPoint(lineIndex: number, lines: string[], structure: any): boolean {
    const line = lines[lineIndex]
  ...
  async copySelection(
    ...
    try {
      ...
      const copyAction: CopyAction = {
        ...
        type: 'selection',
        content: selection,
        tokens: this.estimateTokens(selection),
        timestamp: new Date(),
        source,
        metadata: {
          startLine,
  ...
  private addToCopyHistory(action: CopyAction): void {
    ...
    if (this.copyHistory.value.length > 50) {
      this.copyHistory.value = this.copyHistory.value.slice(-50)
    }
  }
  ...
  private estimateTokens(text: string): number {
    ...
    // Rough estimation: average of word-based and character-based estimates
    const wordBasedTokens = words * 1.3 // Roughly 1.3 tokens per word
    const charBasedTokens = chars / 4   // Roughly 4 characters per token
  ...
  private extractFileSegments(content: string): FileSegment[] {
  ...
  private chunkByNaturalBreaks(
    ...
    return this.chunkBalanced(content, lines, config, structure)
  }

  private chunkByTokenLimit(
    content: string,
    lines: string[],
    config: ChunkingOptions
  ): ContextChunk[] {

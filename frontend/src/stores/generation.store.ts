...
export const useGenerationStore = defineStore("generation", () => {
  ...
  const canGenerate = computed(() => !!contextBuilderStore.currentContext?.content && !!userTask.value.trim());
  ...
  async function suggestContext() {
    ...
    try {
      ...
      treeStateStore.selectFilesByRelPaths(suggestedFiles, fileTreeStore.nodesMap as Map<string, any>);
      uiStore.addToast(`Предложено ${suggestedFiles.length} файлов. Добавлены в контекст.`, "success");
    } catch (err: any) {

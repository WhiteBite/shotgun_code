<template>
  <div class="w-full h-full overflow-y-auto bg-gray-900 flex justify-center p-8">
    <div class="w-full max-w-xl mx-auto space-y-8">
      <div>
        <h1 class="text-3xl font-bold text-center mb-2 text-white">Select a Project</h1>
        <p class="text-center text-gray-400">Choose a recent project or open a new folder to get started.</p>
      </div>

      <div class="bg-gray-800/50 p-6 rounded-lg border border-gray-700 space-y-4">
        <button @click="projectStore.selectProjectFromDialog()" class="w-full flex items-center justify-center gap-3 p-4 bg-blue-600 hover:bg-blue-500 rounded-lg text-white font-semibold text-lg transition-colors shadow-lg">
        ...
        <div v-if="projectStore.recentProjects.length > 0">
          ...
          <h3 class="text-sm font-semibold text-gray-400 mb-2 px-1">Recent Projects</h3>
          <div class="space-y-2">
            <div v-for="proj in projectStore.recentProjects" :key="proj.id"
                 @click="projectStore.selectProject(proj)"
                 class="p-3 bg-gray-700/50 hover:bg-gray-700 rounded-md cursor-pointer border border-transparent hover:border-blue-500 transition-all">

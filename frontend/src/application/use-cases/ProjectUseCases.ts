...
export class SelectProjectDirectoryUseCase {
  ...
  async execute(): Promise<string> {
    try {
      ...
      const isValid = await this.projectRepository.validatePath(selectedPath);
      if (!isValid) {
...
export class ReadFileContentUseCase {
  ...
  async execute(rootPath: string, relativePath: string): Promise<string> {
    ...
    const isValidRoot = await this.projectRepository.validatePath(rootPath);
    if (!isValidRoot) {
      throw new Error('Root path is not accessible');
    }

    // 3. Check if file exists
    const fileExists = await this.projectRepository.fileExists(rootPath, relativePath);
    if (!fileExists) {
...
export class StartFileWatchingUseCase {
  ...
  async execute(
    ...
    const isValid = await this.projectRepository.validatePath(rootPath);
    if (!isValid) {
      throw new Error('Root path is not accessible');
    }

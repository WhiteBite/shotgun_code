...
import { defaultTokenEstimationService, type TokenEstimationService } from '@/domain/services';
...
export const useExportStore = defineStore("export", () => {
  ...
  const tokenEstimationService: TokenEstimationService = defaultTokenEstimationService;

  const isOpen = ref(false);
  const isLoading = ref(false);

  const exportFormat = ref<"plain" | "manifest" | "json">("manifest");
  const stripComments = ref(APP_CONFIG.performance.streaming.STRIP_COMMENTS);
  const includeManifest = ref(APP_CONFIG.performance.streaming.INCLUDE_MANIFEST);

  const aiProfile = ref("Claude-3");

  const estimator = createTokenEstimator();
  const splitter = createSplitterService();
  const tokenLimit = ref(APP_CONFIG.performance.tokens.MAX_TOKENS_PER_REQUEST);
  const fileSizeLimitKB = ref(APP_CONFIG.performance.limits.MAX_CACHE_SIZE / 1024); // Convert to KB

  const enableAutoSplit = ref(true);
  // Use centralized configuration for splitting defaults
  const maxTokensPerChunk = computed(() => 
    settingsStore.settings.maxTokensPerChunk || APP_CONFIG.performance.streaming.CHUNK_SIZE
  );
  const overlapTokens = computed(() => 
    settingsStore.settings.overlapTokens || Math.round(APP_CONFIG.performance.streaming.CHUNK_SIZE * 0.1)
  );
  ...
  function computeSplitPreview() {
    ...
    try {
      ...
      uiStore.addToast(`Split preview failed: ${errorMessage}`, "error");
      splitPreview.value = null;
    }
  }
  ...
  async function copy(req: CopyRequest) {
    ...
    try {
      ...
      const errorMessage = e instanceof Error ? e.message : String(e);
      uiStore.addToast(`Copy failed: ${errorMessage}`, "error");
    }
  }
  ...
  async function doExportClipboard() {
    ...
    try {
      ...
      uiStore.addToast(`Ошибка экспорта: ${errorMessage}`, "error");
    } finally {
  ...
  async function doExportAI() {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      uiStore.addToast(
        `Ошибка AI экспорта: ${errorMessage}`,
        "error",
      );
    } finally {
      isLoading.value = false;

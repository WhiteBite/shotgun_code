...
export const useIntelligentAIStore = defineStore("intelligentAI", () => {
  ...
  async function generateIntelligentCode(
    ...
    try {
      ...
      // Show progress in UI
      console.log('Выполняется интеллектуальная генерация...');

      // Use AIRepository instead of apiService
      const resultJson = await aiRepository.generateIntelligentCode(
        task,
        context,
  ...
  async function generateCodeWithOptions(
    ...
    try {
      ...
      // Show progress in UI
      console.log('Генерация кода с опциями...');

      // Use AIRepository instead of apiService
      const result = await aiRepository.generateCodeWithOptions(
        systemPrompt,
        userPrompt,
        JSON.stringify(finalOptions),
      );
  ...
  async function loadProviderInfo() {
    try {
      // Use AIRepository instead of apiService
      const infoJson = await aiRepository.getProviderInfo();
  ...
  async function loadAvailableModels() {
    try {
      // Use AIRepository instead of apiService
      availableModels.value = await aiRepository.listAvailableModels();
      notifications.addLog(

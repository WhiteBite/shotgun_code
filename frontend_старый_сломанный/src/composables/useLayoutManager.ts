...
export const useLayoutManagerStore = defineStore('layoutManager', () => {
  return createStoreWithDependencies('layoutManager', (dependencies: StoreDependencies) => {
    ...
    // Replace useLocalStorage with StorageRepository
    const presets = ref<Record<string, LayoutPreset>>({})
    
    // Load presets from storage
    const loadPresetsFromStorage = async () => {
      try {
        const storedPresets = localStorageService.get<Record<string, LayoutPreset>>('shotgun-layout-presets', {});
        if (storedPresets) {
    ...
    const savePresetsToStorage = async () => {
      try {
        localStorageService.set('shotgun-layout-presets', presets.value);
      } catch (error) {
    ...
    const calculateInitialPosition = (config: PanelConfig): PanelPosition => {
      ...
      if (isPositionOccupied(position, config.id)) {
        position = findOptimalPosition(config.minSize)
      }
    ...
    const constrainToViewport = (position: PanelPosition): PanelPosition => {
      const space = availableSpace.value
      const { x: origX, y: origY, width: origWidth, height: origHeight } = position
      
      // Constrain to available space
      const x = Math.max(space.x, Math.min(origX, space.x + space.width - origWidth))
      const y = Math.max(space.y, Math.min(origY, space.y + space.height - origHeight))
      
      return { x, y, width: origWidth, height: origHeight }
    }
    ...
    const tileHorizontally = (panels: PanelState[], space: { width: number; height: number; x: number; y: number }): void => {
    ...
    const tileVertically = (panels: PanelState[], space: { width: number; height: number; x: number; y: number }): void => {
    ...
    const tileInGrid = (panels: PanelState[], space: { width: number; height: number; x: number; y: number }): void => {
    ...
    // Watch presets for changes and save to storage
    watch(presets, () => {
      savePresetsToStorage()
    }, { deep: true })

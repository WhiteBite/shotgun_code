...
export const useContextualTooltipStore = defineStore('contextualTooltip', () => {
  ...
  const generateProjectAwareContent = (baseContent: string) => {
    ...
    } else if (context.tokenCount > APP_CONFIG.performance.tokens.MAX_TOKENS_PER_REQUEST) {
  ...
  const generateSmartTips = (elementType: string, context: TooltipContext) => {
    ...
    if (context.fileCount > APP_CONFIG.ui.selection.MAX_CASCADE_FILES) {
      tips.push('🚀 Try using file filters to manage large projects')
    }
    
    if (context.tokenCount > APP_CONFIG.performance.tokens.MAX_TOKENS_PER_REQUEST * 0.8) {

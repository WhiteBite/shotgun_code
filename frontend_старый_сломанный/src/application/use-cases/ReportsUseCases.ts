...
export class ListReportsUseCase {
  ...
  private validateListFilters(filters?: unknown): void {
    if (!filters) return;
...
export class GenerateProjectAnalysisReportUseCase {
  ...
  private validateProjectAnalysisInputs(projectPath: string, options?: unknown): void {
    if (!projectPath?.trim()) {
  ...
  private async analyzeProject(projectPath: string, options?: unknown): Promise<unknown> {
  ...
  private async getGitStats(): Promise<unknown> {
...
export class GenerateCodeQualityReportUseCase {
  ...
  private validateCodeQualityInputs(projectPath: string, filePaths?: string[]): void {
    if (!projectPath?.trim()) {
      throw new Error('Project path is required');
    }
  ...
  private async analyzeCodeQuality(projectPath: string, filePaths?: string[]): Promise<unknown> {
  ...
  private calculateQualitySummary(analysisResult: unknown): unknown {
    const issues = (analysisResult as { issues?: unknown[] }).issues || [];
    const critical = issues.filter((i: unknown) => (i as { severity?: string }).severity === 'critical').length;
    const major = issues.filter((i: unknown) => (i as { severity?: string }).severity === 'major').length;
    const minor = issues.filter((i: unknown) => (i as { severity?: string }).severity === 'minor').length;
...
export class ExportReportUseCase {
  ...
  private async formatReportForExport(report: GenericReport, format: string, options?: { includeRawData?: boolean; includeMetadata?: boolean; compress?: boolean }): Promise<string> {
    switch (format) {
      case 'json': {
        const exportObject = {

...
export class ValidateGitRepositoryUseCase {
  ...
  async execute(projectPath: string): Promise<{
    ...
    try {
      ...
      result.errors.push(new Error('Unknown validation error').message);
    }
  ...
  async execute(projectPath: string): Promise<{
    ...
    try {
      ...
      try {
        ...
        console.warn('Failed to get remote info:', _error);
        info.remoteInfo = { hasRemote: false };
      }

    } catch (_error) {
      console.error('Failed to get git repository info:', _error);
      throw new Error(`Failed to get git repository info: ${_error instanceof Error ? _error.message : 'Unknown error'}`);
    }

...
import type { TokenEstimationService } from '@/domain/services/TokenEstimationService';
import type { CodeValidationService } from '@/domain/services/CodeValidationService';
import { defaultTokenEstimationService, defaultCodeValidationService } from '@/domain/services';
...
export class GenerateCodeUseCase {
  constructor(
    ...
    private tokenEstimationService: TokenEstimationService = defaultTokenEstimationService,
    private codeValidationService: CodeValidationService = defaultCodeValidationService
  ) {}
  ...
  async execute(request: AICodeGenerationRequest): Promise<AICodeGenerationResponse> {
    ...
    try {
      ...
      await this.validateCodeGenerationResponse(response);
  ...
  private async validateCodeGenerationRequest(request: AICodeGenerationRequest): Promise<void> {
    if (!request.prompt?.trim()) {
      throw new Error('Prompt is required for code generation');
    }
    
    // Use centralized configuration for limits
    const maxPromptLength = APP_CONFIG.performance.limits.MAX_CONSOLE_LOGS * 100; // Conservative estimate
    if (request.prompt.length > maxPromptLength) {
      throw new Error(`Prompt is too long (maximum ${maxPromptLength} characters)`);
    }
    
    // Estimate tokens using domain service
    const estimatedTokens = await this.tokenEstimationService.estimateTokens(request.prompt, 'typescript');
    const maxTokens = APP_CONFIG.performance.tokens.MAX_TOKENS_PER_REQUEST;
    
    if (estimatedTokens > maxTokens) {
      throw new Error(`Prompt estimated to use ${estimatedTokens} tokens, which exceeds the limit of ${maxTokens}`);
    }
    
    if (request.maxTokens && (request.maxTokens < 1 || request.maxTokens > maxTokens)) {
      throw new Error(`Max tokens must be between 1 and ${maxTokens}`);
    }
  ...
  private async validateCodeGenerationResponse(response: AICodeGenerationResponse): Promise<void> {
    if (!response.code && !response.analysis) {
      throw new Error('Invalid response: no code or analysis generated');
    }
    
    if (response.code) {
      // Use domain service for code validation
      const validationResult = await this.codeValidationService.validateSyntax(response.code, 'typescript');
      
      if (!validationResult.isValid) {
        console.warn('Generated code has syntax issues:', validationResult.errors);
      }
      
      // Check size using centralized configuration
      const maxCodeSize = APP_CONFIG.performance.limits.MAX_CACHE_SIZE * 50; // Conservative estimate
      if (response.code.length > maxCodeSize) {
        console.warn(`Generated code is very large (${response.code.length} characters), consider breaking it into smaller parts`);
      }
    }
  }
}
...
export class AnalyzeCodeUseCase {
  ...
  private validateCodeAnalysisInputs(code: string, analysisType: string): void {
    ...
    // Use centralized configuration for limits
    const maxCodeSize = APP_CONFIG.performance.limits.MAX_CACHE_SIZE * 100;
    if (code.length > maxCodeSize) {
      throw new Error(`Code is too large for analysis (maximum ${maxCodeSize} characters)`);
    }
...
export class ExplainCodeUseCase {
  ...
  private validateCodeExplanationInputs(code: string): void {
    ...
    // Use centralized configuration for limits
    const maxCodeSize = APP_CONFIG.performance.limits.MAX_CACHE_SIZE * 50;
    if (code.length > maxCodeSize) {
      throw new Error(`Code is too large for explanation (maximum ${maxCodeSize} characters)`);
    }
  }
}
...
export class SuggestContextFilesUseCase {
  ...
  async execute(
    ...
    try {
      // Limit and prioritize suggestions using centralized configuration
      const maxSuggestions = options?.maxSuggestions || APP_CONFIG.performance.limits.MAX_SELECTED_PATHS;
...
export class SmartCodeAssistantUseCase {
  ...
  private validateSmartAssistantInputs(action: string, input: unknown): void {
  ...
  private generateSuggestions(action: string, input: unknown): string[] {

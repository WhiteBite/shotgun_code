...
export class CreateStreamingContextUseCase {
  ...
  async execute(
    ...
    const streamingOptions = {
      ...options,
      useStreaming: true,
      // Force memory-safe options for streaming using APP_CONFIG
      stripComments: options?.stripComments ?? APP_CONFIG.context.streaming.DEFAULT_STRIP_COMMENTS,
      includeManifest: options?.includeManifest ?? APP_CONFIG.context.streaming.DEFAULT_INCLUDE_MANIFEST
    };

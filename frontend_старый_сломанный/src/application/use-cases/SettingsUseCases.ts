  ...
  async execute(settingsJson: string, options?: {
    ...
    try {
      ...
      if (options?.backupCurrent) {
        try {
          const currentSettings = await this.settingsRepo.getSettings();
          // In a real implementation, you'd save this backup somewhere
          console.log(`Current settings backed up (version: ${currentSettings.version})`);
        } catch {
      ...
      throw new Error(`Failed to import settings: ${_error instanceof Error ? _error.message : 'Unknown error'}`);
    }
  }

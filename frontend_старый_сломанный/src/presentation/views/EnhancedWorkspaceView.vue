<template>
  <main class="enhanced-workspace" :class="workspaceClasses">
    ...
    <div class="workspace-content">
      <!-- File Panel -->
      <FilePanelModern
        :is-collapsed="filePanelState.isCollapsed"
      ...
      <div class="workspace-main-content">
        <!-- Manual Mode -->
        <div v-if="workspaceStore.currentMode === 'manual'" class="manual-mode-content">
          ...
          >
            ...
            <template #context-preview>
              ...
              >
                <StreamingContextViewer :context-id="contextStore.currentContext?.id" />
              </BasePanel>
            </template>
          </ResponsiveLayoutGrid>
        </div>
        ...
        <!-- Autonomous Mode -->
        <div v-else-if="workspaceStore.currentMode === 'autonomous'" class="autonomous-mode-content">
          <BasePanel
        ...
        <!-- Reports Mode -->
        <div v-else-if="workspaceStore.currentMode === 'reports'" class="reports-mode-content">
          <BasePanel
      ...
      <ContextBuilder
        :is-collapsed="contextPanelState.isCollapsed"
    ...
    <Transition name="drawer-slide-right">
      <SettingsDrawer 
        ...
        :is-open="uiStore.drawers.settings"
    ...
    <Transition name="modal-fade">
      <ExportModal v-if="uiStore.modals.export" @close="uiStore.closeModal('export')" />
    </Transition>
    
    <Transition name="modal-fade">
      <CommitHistoryModal v-if="uiStore.modals.commitHistory" @close="uiStore.closeModal('commitHistory')" />
    </Transition>
    
    <Transition name="modal-fade">
      <ReportViewerModal v-if="uiStore.modals.reportViewer" @close="uiStore.closeModal('reportViewer')" />
    </Transition>
...
import ContextBuilder from '@/presentation/components/panels/ContextBuilder.vue'
...
// Panel managers
const filePanelManager = usePanelManager('file', 320)
const contextPanelManager = usePanelManager('context', 400)

// Panel states
const filePanelState = ref({
  isCollapsed: filePanelManager.isCollapsed.value
})

const contextPanelState = ref({
  isCollapsed: contextPanelManager.isCollapsed.value
})

// Computed
const workspaceClasses = computed(() => [
  `workspace-mode-${workspaceStore.currentMode}`,
  {
...
const allPanelsCollapsed = computed(() => {
  return filePanelState.value.isCollapsed && contextPanelState.value.isCollapsed
})
...
function handlePanelToggle(panelName: string, collapsed: boolean) {
  if (panelName === 'file') {
    filePanelState.value.isCollapsed = collapsed
  } else if (panelName === 'context') {
    contextPanelState.value.isCollapsed = collapsed
  }
}

function handlePanelResize(panelName: string, width: number) {
  // Panel resize handling
  console.log(`Panel ${panelName} resized to ${width}px`)
}
...
function handleGlobalKeydown(event: KeyboardEvent) {
  ...
  if (event.ctrlKey || event.metaKey) {
    switch (event.key) {
      ...
      case 'b':
        event.preventDefault()
        handlePanelToggle('file', !filePanelState.value.isCollapsed)
        break
      case 'k':
        event.preventDefault()
        handlePanelToggle('context', !contextPanelState.value.isCollapsed)
        break
      case '`':
        event.preventDefault()
...
.workspace-content {
  display: grid;
  grid-template-columns: minmax(280px, 1fr) 2fr minmax(320px, 1.5fr);
  flex: 1;
  min-height: 0;
  gap: 12px;
  padding: 12px;

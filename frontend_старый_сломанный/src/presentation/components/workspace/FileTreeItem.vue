...
const titleText = computed(() => {
  ...
  // CRITICAL: Apply null safety check per memory guidance
  const size = props.node.size ?? 0;
  return `${props.node.relPath} • ${size.toLocaleString()} B`;
});
...
function toggleDir(_: MouseEvent) {
...
function onToggleSelection(_: Event) {

<template>
  ...
  >
    ...
    <div class="p-6 border-b border-gray-700 bg-gray-800/50">
      <div class="space-y-6">
        ...
        <div>
          ...
          <textarea
            v-model="autonomousControlService.currentTask"
            ...
            :disabled="autonomousControlService.isTaskRunning"
          ></textarea>
        </div>
        ...
        <div>
          ...
          <div class="grid grid-cols-3 gap-3">
            <button
              v-for="policy in autonomousControlService.slaPolicies"
              :key="policy.value"
              :class="[
                'relative p-4 rounded-lg border-2 transition-all duration-200 transform hover:scale-105',
                autonomousControlService.slaPolicy === policy.value
                  ? 'border-blue-500 bg-blue-500/10 shadow-lg shadow-blue-500/25'
                  : 'border-gray-600 bg-gray-700 hover:border-gray-500 hover:bg-gray-600',
              ]"
              :disabled="autonomousControlService.isTaskRunning"
              @click="autonomousControlService.slaPolicy = policy.value"
            >
              ...
              <div
                v-if="autonomousControlService.slaPolicy === policy.value"
        ...
        <div class="flex gap-3">
          <div class="text-xs text-gray-500 mb-2">
            Debug: canStartTask={{ autonomousControlService.canStartTask }}, 
            isTaskRunning={{ autonomousControlService.isTaskRunning }}, 
            currentTask.length={{ autonomousControlService.currentTask.length }}
          </div>
          <button
            :disabled="
              !autonomousControlService.canStartTask || autonomousControlService.isLoading
            "
          ...
          >
            <svg
              v-if="autonomousControlService.isLoading"
            ...
            {{
              autonomousControlService.isLoading
                ? "Starting..."
          ...
          <button
            :disabled="!autonomousControlService.canCancelTask"
    ...
    <!-- Task Status -->
    <div
      v-if="autonomousControlService.taskStatus"
    ...
    >
      <div class="space-y-4">
        <div class="flex items-center justify-between">
          ...
          <span :class="autonomousControlService.statusBadgeClass">
            {{ autonomousControlService.taskStatus.status }}
          </span>
        </div>

        <!-- Progress Bar -->
        <div v-if="autonomousControlService.isTaskRunning" class="space-y-3">
          <div class="flex justify-between text-sm text-gray-300">
            <span>Progress</span>
            <span class="font-semibold"
              >{{ Math.round(autonomousControlService.taskStatus.progress) }}%</span
            >
          </div>
          <div class="w-full bg-gray-700 rounded-full h-3 overflow-hidden">
            <div
              ...
              :style="{ width: `${autonomousControlService.taskStatus.progress}%` }"
            ></div>
          </div>
          <div
            v-if="autonomousControlService.taskStatus.currentStep"
          ...
          >
            ...
            Current step: {{ autonomousControlService.taskStatus.currentStep }}
          </div>
          <div
            v-if="autonomousControlService.taskStatus.estimatedTimeRemaining"
            class="text-sm text-gray-400 flex items-center gap-2"
          >
            <svg
            ...
            {{ autonomousControlService.formatTime(autonomousControlService.taskStatus.estimatedTimeRemaining) }}
          </div>
        </div>

        <!-- Error Display -->
        <div
          v-if="autonomousControlService.taskStatus.error"
        ...
        >
          <div class="flex items-center gap-2">
            ...
            <p class="text-red-300 text-sm">
              {{ autonomousControlService.taskStatus.error }}
            </p>
          </div>
        </div>
      </div>
    ...
    <div v-if="autonomousControlService.tplPlan" class="flex-1 p-6 overflow-y-auto">
      ...
      <div class="space-y-4">
        <div
          v-for="step in autonomousControlService.tplPlan.steps"
          :key="step.id"
          class="p-4 bg-gray-700/50 rounded-lg border-l-4 transition-all duration-200 hover:bg-gray-700/70"
          :class="autonomousControlService.stepStatusBorderClass(step.status)"
        >
          <div class="flex items-center justify-between mb-3">
            <h4 class="font-semibold text-white flex items-center gap-2">
              <svg
                class="w-4 h-4"
                :class="autonomousControlService.stepStatusIconClass(step.status)"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
            ...
            <span :class="autonomousControlService.stepStatusBadgeClass(step.status)">
              {{ step.status }}
            </span>
          </div>
          ...
          >
            ...
            Duration: {{ autonomousControlService.formatDuration(step.duration) }}
          </div>
          <div
            v-if="step.error"
    ...
    <!-- Reports Section -->
    <div
      v-if="autonomousControlService.reports.length > 0"
    ...
    >
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        <div
          v-for="report in autonomousControlService.reports"
          :key="report.id"
          ...
          @click="autonomousControlService.handleViewReport(report)"
        >
          <div class="flex items-center justify-between mb-3">
          ...
          <div class="text-xs text-gray-500 flex items-center gap-1">
            ...
            {{ autonomousControlService.formatDate(report.createdAt) }}
          </div>
        </div>
      </div>
    </div>
    ...
    <!-- Error Display -->
    <div
      v-if="autonomousControlService.error"
      ...
      <div class="p-4 bg-red-900/30 border border-red-700 rounded-lg">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-2">
            ...
            <p class="text-red-300 text-sm">{{ autonomousControlService.error }}</p>
          </div>
          <button
            class="text-xs text-red-400 hover:text-red-300 transition-colors"
            @click="autonomousControlService.handleClearError()"
          >
            Dismiss
          </button>
        </div>
...
import { useAutonomousControlService } from "@/composables/useAutonomousControlService";

const { autonomousControlService, autonomousStore, projectStore, notificationsStore, selectedSlaPolicy } = useAutonomousControlService();

async function startTask() {
  if (!projectStore.currentProject) {
    notificationsStore.addLog("No project loaded", "error");
    return;
  }

  const success = await autonomousControlService.handleStartTask(
    projectStore.currentProject.path,
  );
  if (success) {
    notificationsStore.addLog("Autonomous task started successfully", "success");
  }
}

async function cancelTask() {
  const success = await autonomousControlService.handleCancelTask();
  if (success) {
    notificationsStore.addLog(`Task cancellation requested`, "info");
  }
}
</script>
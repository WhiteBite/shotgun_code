    ...
    <div class="split-status-bar">
      ...
      <div v-if="copyHistory.length > 0" class="copy-history">
        ...
        <div class="history-items">
          ...
          >
            {{ item.type === 'chunk' ? `Chunk ${(item.chunkIndex ?? 0) + 1}` : 'All' }}
          </span>
        </div>
      </div>
    </div>
  ...
  if (!props.context?.content || !props.splitSettings.enabled) {
    ...
    return [{
      ...
      metadata: {
        chunkIndex: 0,
        totalChunks: 1,
        linesCount: countLines(content),
        significance: 'high' as const,
        ...(props.context?.metadata && typeof props.context.metadata === 'object' ? props.context.metadata : {})
      }
    }]
  }

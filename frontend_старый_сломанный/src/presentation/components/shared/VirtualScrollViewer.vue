...
const props = withDefaults(defineProps<Props>(), {
  ...
  height: APP_CONFIG.ui.virtualScroll.ITEM_HEIGHT * 20, // Default 20 lines visible
  lineHeight: APP_CONFIG.ui.virtualScroll.LINE_HEIGHT,
  overscan: APP_CONFIG.ui.virtualScroll.VIEWER_OVERSCAN, // Reduced overscan for better performance
  totalLines: 0
})
...
const getLine = (index: number): string => {
  ...
  const maxLines = APP_CONFIG.ui.virtualScroll.VIEWER_MAX_LINES // Hard limit to prevent memory issues
  if (index >= maxLines) {
    if (index === maxLines) {
...
const totalLineCount = computed(() => {
  if (props.totalLines && props.totalLines > 0) {
    return Math.min(props.totalLines, APP_CONFIG.ui.virtualScroll.VIEWER_MAX_LINES) // Hard limit
  }
  ...
  const maxLines = APP_CONFIG.ui.virtualScroll.VIEWER_MAX_LINES // Hard limit
  const newlineRegex = /\n/g
  // let match
  let lineCount = 0
  
  while ((newlineRegex.exec(props.content)) !== null) {
    lineCount++
...
const visibleLines = computed(() => {
  ...
  const maxVisibleLines = APP_CONFIG.ui.virtualScroll.VIEWER_MAX_VISIBLE_LINES
...
const loadVisibleContent = async () => {
  ...
  if (linesToLoad.length > 0) {
    try {
      ...
      const chunkSize = APP_CONFIG.ui.virtualScroll.VIEWER_CHUNK_SIZE
...
// Performance monitoring with AGGRESSIVE thresholds - REDUCED for memory safety
const memoryUsage = ref(0)
const MAX_CACHE_SIZE = APP_CONFIG.ui.virtualScroll.VIEWER_MAX_CACHE_SIZE // CRITICAL: Only keep 1 chunk in memory (reduced from 3)
const MEMORY_CLEANUP_INTERVAL = APP_CONFIG.ui.virtualScroll.VIEWER_MEMORY_CLEANUP_INTERVAL // Cleanup every 1 second (reduced from 3 seconds)
const CRITICAL_MEMORY_THRESHOLD = APP_CONFIG.ui.virtualScroll.VIEWER_CRITICAL_MEMORY_THRESHOLD // 10MB threshold (reduced from 20MB)
...
// CRITICAL: Ultra-aggressive cache cleanup function
const aggressiveCacheCleanup = () => {
  console.log('Performing ultra-aggressive cache cleanup in VirtualScrollViewer');
  
  // Only keep the most recent MAX_CACHE_SIZE chunks (ultra-reduced)
  if (cachedLines.value.size > MAX_CACHE_SIZE * APP_CONFIG.ui.virtualScroll.VIEWER_MAX_VISIBLE_LINES) { // Reduced from 50 to 25
    const entries = Array.from(cachedLines.value.entries());
    cachedLines.value.clear();
    
    // Keep only the last MAX_CACHE_SIZE * 25 lines (ultra-reduced)
    const recentEntries = entries.slice(-MAX_CACHE_SIZE * APP_CONFIG.ui.virtualScroll.VIEWER_MAX_VISIBLE_LINES);
    recentEntries.forEach(([key, value]) => {
      cachedLines.value.set(key, value);
    });
    
    console.log(`Ultra-cleaned cache: ${entries.length} -> ${recentEntries.length} lines`);
  }
  ...
  if (window.gc) {
    try {
      window.gc();
      console.log('Forced garbage collection in ultra-aggressive cleanup');
    } catch (_e) {
      console.warn('Failed to trigger garbage collection in VirtualScrollViewer:', _e);
    }
  }
}
...
onMounted(() => {
  updateMemoryUsage()
  // CRITICAL: Ultra-frequent memory monitoring and ultra-aggressive cleanup
  const memoryInterval = setInterval(() => {
    updateMemoryUsage();
    // Check memory every second and trigger cleanup if needed
    if (memoryUsage.value > CRITICAL_MEMORY_THRESHOLD * (APP_CONFIG.ui.virtualScroll.VIEWER_MEMORY_CLEANUP_THRESHOLD / 10)) { // Start cleanup at 50% of threshold
      aggressiveCacheCleanup();
    }
  }, APP_CONFIG.ui.virtualScroll.VIEWER_MEMORY_CHECK_INTERVAL); // Check every 500ms
  ...
  onUnmounted(() => {
    ...
    if (window.gc) {
      try {
        ...
        console.warn('Failed to trigger final garbage collection:', _e);
      }
    }
  })
  ...
  clearCache: () => {
    ...
    if (window.gc) {
      try {
        ...
        console.warn('Failed to trigger manual garbage collection:', _e);
      }
    }
  },

<template>
  <transition name="modal-fade">
    <div
      v-if="commitHistoryService.isHistoryVisible"
      class="fixed inset-0 bg-black/60 z-40 flex items-center justify-center"
      role="dialog"
      aria-modal="true"
      @click.self="commitHistoryService.cancelSelection"
    >
      <div
      ...
      >
        ...
        >
          ...
          <button
            ...
            aria-label="Close"
            @click="commitHistoryService.cancelSelection"
          >
            <svg
        ...
        <main class="p-4 overflow-y-auto flex-grow">
          <div v-if="commitHistoryService.isLoading" class="text-center text-gray-400">
            {{ t("commits.loading") }}
          </div>
          <div
            v-else-if="commitHistoryService.commits.length === 0"
          ...
          <div v-else class="space-y-2">
            <div
              v-for="commit in commitHistoryService.commits"
              :key="commit.hash"
              class="p-3 bg-gray-900/50 rounded-md border border-gray-700"
            >
              <div class="flex gap-3">
                <input
                  :model-value="commitHistoryService.getSelectedCommits().includes(commit.hash)"
                  type="checkbox"
                  :value="commit.hash"
                  class="form-checkbox mt-1.5 bg-gray-700 border-gray-500 rounded text-blue-500"
                  @change="commitHistoryService.toggleCommitSelection(commit.hash)"
                />
                <div class="flex-grow min-w-0">
                  ...
                  <div class="mt-2 space-y-0.5 text-xs text-gray-300">
                    <div
                      ...
                      @click="commitHistoryService.openPinnedFile(file, commit.hash, $event)"
                      @mouseenter="commitHistoryService.showFilePreview(file, commit.hash, $event)"
                      @mouseleave="commitHistoryService.hideFilePreview"
                    >
                      {{ file }}
                    </div>
                  </div>
        ...
        >
          <span class="text-sm text-gray-400"
            >{{ commitHistoryService.selectedCommitsCount }} commit(s) selected</span
          >
          <div class="flex gap-3">
            <button
            ...
            <button
              :disabled="!commitHistoryService.canApplySelection"
              class="px-4 py-2 bg-blue-600 hover:bg-blue-500 rounded-md disabled:opacity-50"
              @click="commitHistoryService.applySelection"
            >
              {{ t("button.addFiles") }}
            </button>
          </div>
...
import { useCommitHistoryService } from "@/composables/useCommitHistoryService";

const { commitHistoryService, gitStore, uiStore, projectStore } = useCommitHistoryService();
</script>
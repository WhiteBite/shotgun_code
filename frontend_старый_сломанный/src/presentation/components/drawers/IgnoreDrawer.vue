<template>
  <transition name="slide-fade">
    <div
      v-if="uiStore.drawers.ignore"
      class="fixed inset-0 z-30"
      role="dialog"
      aria-modal="true"
              @click.self="uiStore.closeDrawer('ignore')"
    >
      <aside
        class="absolute top-0 right-0 h-full w-96 bg-gray-800 border-l border-gray-700 shadow-2xl p-4 flex flex-col"
      >
        <div class="flex items-center justify-between mb-4 flex-shrink-0">
          ...
          <button
            ...
            aria-label="Close"
            @click="uiStore.closeDrawer('ignore')"
          >
            <svg
        ...
        <div class="flex-grow flex flex-col min-h-0 space-y-3">
          <p class="text-xs text-gray-400">
            Используйте glob-паттерны для исключения файлов и папок. Каждое
            правило с новой строки.
          </p>
          <textarea
          ...
          <div class="mt-2">
            <button
              class="px-2 py-1 text-xs bg-gray-700 hover:bg-gray-600 rounded-md"
              @click="toggleGitignore"
            >
              {{ showGitignore ? "Скрыть .gitignore" : "Показать .gitignore" }}
            </button>
            <div
              v-if="showGitignore"
              class="mt-2 bg-gray-900 border border-gray-700 rounded-md p-2 overflow-auto max-h-40"
            >
              <pre class="text-[11px] text-gray-300 whitespace-pre-wrap">{{
                gitignoreText || "Нет данных .gitignore"
              }}</pre>
            </div>
          </div>
        </div>
...
watch(
  () => uiStore.drawers.ignore,
  (isOpen) => {
    if (isOpen && showGitignore.value) loadGitignore();
  },
);

function onKeydown(e: KeyboardEvent) {
  if (e.key === "Escape") uiStore.closeDrawer('ignore');
}

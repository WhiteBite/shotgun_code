...
import { useSafetySettingsService } from '@/composables/useSafetySettingsService'
import type { SafetySettings } from '@/domain/services/SafetySettingsService'
...
const { safetySettingsService } = useSafetySettingsService()
...
const sanitizeMemoryWarning = () => {
  ...
  localSettings.value.maxMemoryWarning = safetySettingsService.sanitizeMemoryWarning(val)
  updateSettings()
}

const sanitizeContextSize = () => {
  const val = Number(localSettings.value.maxContextSizeMB)
  localSettings.value.maxContextSizeMB = safetySettingsService.sanitizeContextSize(val)
  updateSettings()
}
</script>
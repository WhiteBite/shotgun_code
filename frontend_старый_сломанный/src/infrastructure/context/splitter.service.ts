...
function defaultEstimateTokens(text: string): number {
  ...
  if (!text) return 0;
  return Math.max(1, Math.ceil(text.length / APP_CONFIG.context.splitter.AVG_CHARS_PER_TOKEN));
}
...
function packSegmentsByTokens(
  ...
  const currentStart = segments.length ? segments[0].start : 0;
  let accTokens = 0;
  ...
  for (const seg of segments) {
    ...
    if (segTokens <= limitTokens) {
      ...
      const charsPerToken = avgCharsPerToken || APP_CONFIG.context.splitter.AVG_CHARS_PER_TOKEN;
      const stepChars = Math.max(APP_CONFIG.context.splitter.MIN_STEP_CHARS, Math.floor(limitTokens * charsPerToken));
      let pos = seg.start;
      while (pos < seg.end) {
      ...
      warnings.push(
        `Segment too large, split by token within: ${seg.title || seg.relPath || seg.id}`,
      );
      accTokens = 0;
      lastEnd = 0;
...
export function createSplitterService(): SplitterService {
  return {
    ...
    split(
      ...
      const estimator = (t: string) =>
        opts?.estimator ? opts.estimator(t) : defaultEstimateTokens(t);
      ...
      if (
        ...
        const { chunks, warnings } = packSegmentsByTokens(
          text,
          segments,
          limit,
          estimator,
      ...
      const avgCharsPerToken = totalTokens > 0 ? totalChars / totalTokens : APP_CONFIG.context.splitter.AVG_CHARS_PER_TOKEN;
      const stepChars = Math.max(APP_CONFIG.context.splitter.MIN_STEP_CHARS, Math.floor(limit * avgCharsPerToken));

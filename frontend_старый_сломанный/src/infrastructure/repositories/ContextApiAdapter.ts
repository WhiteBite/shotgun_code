...
export class ContextApiAdapter implements ContextRepository {
  ...
  async getContextContent(id: string, pagination: PaginationOptions): Promise<ContextChunk> {
    ...
    try {
      ...
      // Ensure parameters are numbers for Go backend
      const startLine = parseInt(String(pagination.startLine), 10);
      const lineCount = parseInt(String(pagination.lineCount), 10);
      
      const linesJson = await GetContextLines(id, startLine, lineCount);
  ...
  async getStreamingContextLines(
    ...
    try {
      // Ensure parameters are numbers for Go backend
      const start = parseInt(String(startIndex), 10);
      const end = parseInt(String(endIndex), 10);
      
      const result = await GetContextLines(contextId, start, end);
      return JSON.parse(result);
    } catch (error) {
  ...
  private handleError(error: unknown, context: string): Error {
    ...
    if (message.startsWith('domain_error:')) {
      try {
        ...
        (structuredError as unknown as { code?: string; recoverable?: boolean; context?: unknown; cause?: unknown }).code = domainError.code;
        (structuredError as unknown as { code?: string; recoverable?: boolean; context?: unknown; cause?: unknown }).recoverable = domainError.recoverable;
        (structuredError as unknown as { code?: string; recoverable?: boolean; context?: unknown; cause?: unknown }).context = domainError.context;
        (structuredError as unknown as { code?: string; recoverable?: boolean; context?: unknown; cause?: unknown }).cause = domainError.cause;
